...
ansible-playbook 00_prepare_setup.yaml -e "student=4" -e "k8s_role=master" -e "node_index=1" -e "suffix=$suffix"
ansible-playbook 01_prepare_nodes.yaml
ansible-playbook 02_configure_master.yaml
ansible-playbook 03_deploy_master.yaml
ansible-playbook 11_register_svc.yaml
for m in $(seq 1 $WORKERS); do
  echo "  - Building worker $m"
  ansible-playbook 00_prepare_setup.yaml -e "student=4" -e "k8s_role=worker" -e "node_index=$m" -e "suffix=$suffix"
  ansible-playbook 01_prepare_nodes.yaml
